    ...
    std::shared_ptr<const mesh::Mesh<U>> mesh = nullptr)
{
  ...
  using cuda_kern = std::function<void(
      int*, const char*** , const char***, const char**,
      const char**)>; 
  std::map<IntegralType, std::vector<std::tuple<int, cuda_kern>>> cuda_integral_data;
  // We don't need to re-extract subdomain info
  // We just need to extract all of the cuda code generated by ufcx
  ...
  for (int i = 0; i < num_integrals_type.size(); i++)
  {
    ...
    switch (i)
    {
       case cell:
         ...
         break;
       case exterior_facet:
         type = IntegralType::exterior_facet;
	 break;
       case interior_facet:
         type = IntegralType::interior_facet;
         break;
       default:
         throw std::runtime_error("Unsupported integral type."); 
    }
    
    auto cuda_itg = cuda_integral_data.insert({type, {}});
    for (int j = 0; j < num_integrals_type[i]; j++)
    {
      ...
      cuda_itg.first->second.emplace_back(id, k);
    }
  }
  return Form<T, U>(spaces, integrals, cuda_integral_data, coefficients, constants,
